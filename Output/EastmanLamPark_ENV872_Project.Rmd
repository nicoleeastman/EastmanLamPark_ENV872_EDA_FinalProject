...
```{r, echo=FALSE, warning = FALSE, message =FALSE, fig.cap="Sea Level Trend Exploration", fig.align='left', fig.height= 4}
...
This plot displays the mean sea level measurements from 2010 to 2022 for each station. There are fluctuations in sea level on a month-to-month basis, but trend lines represent a slight increase in sea level at all three sites since 2010. We want to explore if this increasing trend line is statistically significant.
...
The graph shows the mean tide levels for each station, and produced similar results as the mean sea level plot. We noticed fluctuations from month-to-month, but the linear model lines show an increase in tide levels at all the sites. 

\newpage

```{r,echo=FALSE, warning = FALSE, message =FALSE, fig.cap="Sea Level Trend Exploration", fig.align='left', fig.height= 4}
#Sea Level Measurements for 2011 
...
```
The graphs display the mean sea levels in 2011. This graph indicates increasing mean sea levels over the course of the year for all three sites, but there is a decrease of sea level from September for all sites. We want to explore whether this increasing trend is statistically significant and whether seasonality affects this monotoinc trend. 

\newpage
```{r,echo=FALSE, warning = FALSE, message =FALSE, fig.cap="Sea Level Trend Exploration", fig.align='left', fig.height= 4}
#Sea Level Measurements for 2021
...
This graphs display the mean sea levels 2021. Similar to the mean sea levels 2011, this graph indicates increasing mean sea levels over the course of the year for Wrightsville Beach and Beaufort, but decreasing sea levels on Hatteras Island. However, overall increase of sea level is observed from 2021 data compared to that of 2011. Time series result for Hatteras will be intereting since it shows decreasing trend in 2021, unlike in 2011. The trend lines look very similar between the graphs so we are interested in the results produced by the time series analyses for more information.

\newpage

## Water temperature data exploration 

```{r, echo=FALSE, warning = FALSE, message =FALSE, fig.cap="Water Temperature Trend Exploration", fig.align='left', fig.height= 6, fig.width= 8}
...
sites according to the monthly water temperature graph.
Water temperatures are high in the summer seasons and low in the winter
...
The biggest increase in temperature was observed in Beaufort. We want to explore 
if this seasonality affects the overall increase in water temperature and whether 
this increase in water temperature has statistical significant relationship with
sea level increase. 
...
```{r, echo=FALSE,fig.cap="Beaufort Sea Level Timeseries", fig.align='left', fig.height= 4, warning= FALSE}
#Beaufort Sea Level Time Series
...
```{r,echo=FALSE,fig.cap="Hatteras Sea Level Timeseries", fig.align='left', fig.height= 4, warning= FALSE}
#Hatteras Sea Level Time Series
...
```{r,echo=FALSE, fig.cap="Wrightsville Sea Level Timeseries", fig.align='left', fig.height= 4, warning=FALSE}
#Wrightsville Sea Level Time Series
...
```{r,echo=FALSE, fig.cap="Beaufort Water Temperature Timeseries", fig.align='left', fig.height= 4, warning=FALSE}
#Beaufort Water Temperature Time Series
...
```{r,echo=FALSE,warning=FALSE, fig.cap="Hatteras Water Temperature Timeseries", fig.align='left', fig.height= 4}
...
```{r,echo=FALSE, fig.cap="Wrightsville Water Temperature Timeseries", fig.align='left', fig.height= 4, warning=FALSE}
#Wrightsville Water Temperature Time Series
...
statistically significant value of tau (0.1853611) indicates a weak positive correlation between sea level and water temperature. This means that there is a stastically significant relationship between sea level and water temperature. Thus, when there is an increase in sea level, there will be an increase of water temperature and vice versa.

The result for time series analysis and correlation test is interesting because two sites, Hatteras and Wrightsville, did not have statistically significant increase in water temperature overtime. Yet, correlation test indicates there is a statistically significant positive correlation between two parameters. There might be confounding variables that might have affected the correlation test, so further test to see if there is any bias in correlation is recommended for the future research. 
