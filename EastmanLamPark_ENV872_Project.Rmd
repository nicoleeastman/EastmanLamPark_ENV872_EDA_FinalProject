...
6. Analyses
  i) Beaufort Sea Level Time Series
  ii) Hatteras Sea Level Time Series
  iii) Wrightsville Sea Level Time Series
  iv) Beaufort Water Temperature Time Series
  v)  Hatteras Water Temperature Time Series
  vi) Wrightsville Water Temperature Time Series
  vii) Correlation Tests between Sea Level Changes and Water Temperature Changes
...
\newpage
\listoffigures 
figure 1.  Sea Level Trend Exploration
figure 2. Water Temperature Trend Exploration
figure 3. Beaufort Sea Level Time Series Plot
figure 4. Hatteras Sea Level Time Series Plot
figure 5. Wrightsville Sea Level Time Series Plot
figure 6. Beaufort Water Temperature Time Series Plot
figure 7. Hatteras Water Temperature Time Series Plot
...
to analyze the changes occurring on the coast of North Carolina due to climate 
change. We assumed that sea level changes have a relationship with global 
...
those sites are spread out across the North Carolina's coast. 
...
The data begins on the first day of 2011 and ends on November 31, 2022. 
...
The original data had some error and random value of 999 was in between the wtmp
...
summarize functions. 
We created the processed data and saved them as csv files. 

##add sea level wrangling process here 
...
The data was already in in more simplified format in comparison to the water 
...
were joined with the rbind function. 

##add a table summarizing the dataset structure 
...
source_3 <-c("Daily Temperature   ", "  °C", "      NOAA's National Data Buoy Center")
...
```{r, include = FALSE, fig.cap="Sea Level Trend Exploration", fig.align='left', fig.height= 2.5}
...
how many graphs are there--- each graphs need seperate descriptions

```{r,include = FALSE, fig.cap="Water Temperature Trend Exploration", fig.align='left', fig.height= 2.5, fig.width= 5}
...
sites. Water temperatures are high in the summer seasons and low in the winter
...
```{r,include = FALSE,fig.cap="Beaufort Sea Level Timeseries", fig.align='left', fig.height= 2.5}
...
According to the Kendall's rank correlation test, the p-value is (5.949e-08) which is smaller than significance level(0.05). This means that we reject the null hypothesis that tau is equals to 0. Therefore, the tau value (0.1853611) is statistically significant, which indicates there is a weak positive correlation between the two parameters. This means that there is a slight correlation that indicates when one value increases, the other will increases. 
...
From the time series analysis, we observed a increasing trend present
in sea level for all three sites. Beaufort, with seasonality (Mann-Kandall; tau=0.447, p < 2.94e^-13) and without seasonality (Mann-Kandall; tau=0.413, p < 2.22e^-16), both tau values  were statistically significant. In Hatteras, with seasonality (Mann-Kandall; tau=0.274, p < 1.86e^-5) and 
without seasonality (Mann-Kandall; tau=0.273, p < 1.31e^-6), both tau values are significant. 
In Wrightsville, with seasonality (Mann-Kandall; tau=0.242, p < 0.0002) and without seasonality (Mann-Kandall; tau=0.299, p < 2.38e-7), both tau values were statistically significant. 

However, monotonic increase of water temperature 
over time was only observed in Beaufort. Beaufort, with seasonality (Mann-Kandall; tau=0.288, p < 1.96e^-5) and without seasonality (Mann-Kandall; tau=0.228, p < 0.0001), both tau values were statistically significant. In Hatteras, with seasonality (Mann-Kandall; tau=0.0152, p < 0.82 and 
without seasonality (Mann-Kandall; tau=-0.00625, p < 0.92), both tau values are not statistically significant. In Wrightsville, with seasonality (Mann-Kandall; tau=0.006, p < 0.92) and without seasonality (Mann-Kandall; tau=0.116, p < 0.84), both tau values are not statistically significant.  

Sea level has been increasing over all three sites, while only Beaufort has increasing water temperature over time. Seasonality did not play role at three sites in North Carolina. 

From Kandall's rank correlation test, we got p-value of (5.949e-08), thus a
statistically significant value of tau(0.1853611). Since tau is small positive, 
we concluded that there is a weak positive correlation between the sea level and 
water temperature. This indicates that when there is an increase in sea level, 
there will be an increase of water temperature.
